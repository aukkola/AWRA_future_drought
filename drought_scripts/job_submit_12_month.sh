...
PBS_PATH=/g/data/w97/amu561/Steven_CABLE_runs/code/shell/pbs_jobs
mkdir -p ${PBS_PATH}
cd /g/data/w97/amu561/Steven_CABLE_runs/drought_scripts/
#for bcm in MRNBC; do
for bcm in QME MRNBC CCAM ISIMIP2b; do
    ...
    for model in CSIRO-BOM-ACCESS1-0 CNRM-CERFACS-CNRM-CM5 NOAA-GFDL-GFDL-ESM2M MIROC-MIROC5; do
        ...
        for scenario in rcp45 rcp85; do
		  ...
		  outdir="/g/data/w97/amu561/Steven_CABLE_runs/drought_metrics/12-month/${bcm}/${model}/"  
                mkdir -p $outdir
		  out_file=/g/data/w97/amu561/Steven_CABLE_runs/drought_metrics/12-month/${bcm}/${model}/drought_metrics_${bcm}_${model}_${variable}_${scenario}_12.nc 
                     
                if [ ! -f ${out_file} ]; then

                echo job_${bcm}_${model}_${scenario}_${variable}
                cp job_template_12.sh ${PBS_PATH}/job_${bcm}_${model}_${scenario}_${variable}.sh
                wait
