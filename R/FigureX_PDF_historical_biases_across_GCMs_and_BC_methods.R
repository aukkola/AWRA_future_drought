...
unit <- c(duration="months", rel_intensity="%") # (#expression("mm"~"month"^"-1"), expression("no. events 10 yrs"^"-1"))

metric_labels <-c(duration="Duration", rel_intensity="Relative intensity")
...
for (m in 1:length(metrics)) {
  ...
  ### Set up figure ###
  png(paste0(outdir, "/FigureX", "_historical_PDFs_of_gmcs_and_bc_methods_in_", metrics[m], "_",
             percentile, "_", scale, "_", exp, ".png"),
      height=9.5, width=6.3, units="in", res=400)
  
  
  par(mai=c(0, 0.1, 0.2, 0.1))
  par(omi=c(0.1, 0.7, 0.4, 0.3))
  ...
  for (v in 1:length(vars)) {
    ...
    if(v==1)  mtext(side=3, line=1, cex=1.2, "GCM")
      
    
    #Variable label
    mtext(side=2, line=4, cex=1.2, var_labels[v])
    
    #y-label
    mtext(side=2, line=2, cex=0.8, "Fraction of land area (-)")
    ...
    #x-label
    if(v ==3) mtext(line=3, side=1, paste0(metric_labels[metrics[m]], " (", unit[metrics[m]], ")"))
    ...
    #Get plot ranges
    plot_y_range <- range(unlist(bc_freqs), na.rm=TRUE)
