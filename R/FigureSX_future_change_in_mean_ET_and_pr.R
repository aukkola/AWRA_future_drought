...
lims_diff <- list(pr      = c(-1000, -10, -5, -3, -1, 0, 1, 3, 5, 10, 1000),
                  etot    = c(-1000, -30, -20, -10, 5, 0, 5, 10, 20, 30, 1000))

unit <- c("mm/month", "mm/month") # (#expression("mm"~"month"^"-1"), expression("no. events 10 yrs"^"-1"))
...
### Set up figure ###
png(paste0(outdir, "/FigureSX", "_Mean_changes_in_monthly_pr_and_ET", ".png"),
    height=8, width=5.5, units="in", res=400)
...
layout(matrix(c(1:6, 7, 8, 3, 9, 10, 6), nrow=6), heights=c(1 , 1, 0.3,
                                                           1 , 1, 0.3))
...
for (v in 1:length(vars)) {
  ...
  mean_diff_rcp45 <- rcp45_fut_mean - hist_mean
  ...
  for (p in 1:length(plot_data)) {
    ...
    image(plot_data[[p]], breaks=c(lims_diff[[vars[v]]]), 
  ...
  if (v==1) {
    ...
    #Legend
    len <- length(lims_diff[[vars[v]]])-1
    add_raster_legend2(cols=cols_diff(len), limits=lims_diff[[vars[v]]][2:len],
  ...
  sd_diff_rcp45 <- rcp45_fut_sd - hist_sd
