calculate_time_mean <- function(models, outdir_exp, exp, proj, metrics, hist_end=2014, extent_raster)
{

  
  #Set period for calculating mean
  if (exp %in% c("historical", "observed")) {
    out_period <- paste0("1950_", hist_end)
  } else {
    out_period <- "2015_2050_and_2051_2099"
  ...
  for (f in 1:length(files)) {
    ...
    if (exp == "historical"  & proj == "CMIP5" & data_yrs[length(data_yrs)] < hist_end) {
      
      ind <- tail(which(data_yrs==2005), n=1) + 1
      
      data_yrs[ind:length(data_yrs)] <- rep(2006:hist_end, each=12)
    ...
    if (exp %in% c("historical", "observed")) {
      ...
      if (metrics == "frequency") {
        ...
        subset_data <- calc(data[[which(data_yrs == start_yr)[1] : tail(which(data_yrs == hist_end), n=1)]], 
                            fun=calc_freq)
        
      } else {
        
        subset_data <- mean(data[[which(data_yrs == start_yr)[1] : 
                                    tail(which(data_yrs == hist_end), n=1)]], na.rm=TRUE)
      }
